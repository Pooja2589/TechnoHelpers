  ...
  if (!serversLoaded) {
    ...
    var handlePacket = function (msg, rinfo) {
      ...
      for (var midx in metrics) {
        ...
        if (keyFlushInterval > 0) {
          if (! keyCounter[key]) {
            keyCounter[key] = 0;
            keyMessage[key] = ' count= ' + keyCounter[key] + ' key= ' + key + ' value= ' + bits[0];
          }
          keyCounter[key] += 1;
          keyMessage[key] = ' count= ' + keyCounter[key] + ' key= '+ key + ' value= ' + bits[0];
        }
    ...
    if (keyFlushInterval > 0) {
      ...
      keyFlushInt = setInterval(function () {
        ...
        // since this does not show the data value so it might not tell what is actually coming to the statsd and used.
        for (var i = 0, e = sortedKeys.length * (keyFlushPercent / 100); i < e; i++) {
          logMessage += timeString + " count=" + sortedKeys[i][1] + " key=" + sortedKeys[i][0] + "\n";
        }
        for(var key in keyMessage){
          logMessage += timeString + keyMessage[key] + "\n";
        }
